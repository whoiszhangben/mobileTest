namespace SmoONE.Infrastructure.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<SmoONE.Infrastructure.SmoONEDbContext>
    {
        ...
        protected override void Seed(SmoONE.Infrastructure.SmoONEDbContext context)
        {
            ...
            context.ProfileTypes.AddOrUpdate(
                new ProfileType { PT_ID = "SFZFYJ", PT_Name= "身份证复印件" },
                new ProfileType { PT_ID = "XlZS", PT_Name = "学历证书" },
                new ProfileType { PT_ID = "RZDJB", PT_Name = "入职登记表" },
                new ProfileType { PT_ID = "LDHT", PT_Name = "劳动合同" },
                new ProfileType { PT_ID = "SBK", PT_Name = "社保卡" },
                new ProfileType { PT_ID = "LZZM", PT_Name = "离职证明" }
                );
            //在RB_Type表中添加默认数据
